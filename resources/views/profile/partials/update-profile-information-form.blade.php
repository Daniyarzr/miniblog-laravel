<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-4">
        ...
        <div class="mb-3">
            ...
            <x-text-input id="avatar_url" name="avatar_url" type="url" class="mt-1 w-100" :value="old('avatar_url', $user->avatar_url)" autocomplete="avatar_url" />
            <x-input-error class="mt-1" :messages="$errors->get('avatar_url')" />
            @if($user->avatar_url)
                <div class="mt-2">
                    <img src="{{ $user->avatar_url }}" alt="avatar" class="rounded-circle" width="48" height="48">
                </div>
            @endif
        </div>
